...
public class DoodleView extends View {
    ...
    private void touchMoved(MotionEvent event) {
        for (int i = 0; i < event.getPointerCount(); i++) {
            int poinerID = event.getPointerId(i);
            int pointerIndex = event.findPointerIndex(poinerID);

            if (pathMap.containsKey(poinerID)) {
                float newX = event.getX(pointerIndex);
                float newY = event.getY(pointerIndex);

                Path path = pathMap.get(poinerID);
                Point point = previousPointMap.get(poinerID);
